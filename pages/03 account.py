...
if st.button("Next"):
    ...
    if check_in_database(fname, mname, lname):
        ...
        st.write(f"Create new account with name '{fname, mname, lname}'")
    ...
    with st.form('Form1'):
        ...
        if submit:
            ...
            try:
                ...
                cursor.execute("INSERT INTO Customer Values ('CT052', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",(fname, mname, lname, gender, phone, email, dob, city, country, address))
                connection.commit()
